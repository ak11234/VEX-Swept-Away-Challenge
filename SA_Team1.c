#pragma config(Sensor, in2,    LineFollower,   sensorLineFollower)
#pragma config(Sensor, dgtl3,  Sonar,          sensorSONAR_inch)
#pragma config(Motor,  port1,           LeftDrive,     tmotorVex393_HBridge, openLoop, reversed, driveLeft)
#pragma config(Motor,  port2,           RightDrive,    tmotorVex393_MC29, openLoop, driveRight)
#pragma config(Motor,  port3,           ArmMotor,      tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port4,           ActMotor,      tmotorVex393_MC29, openLoop)
//*!!Code automatically generated by 'ROBOTC' configuration wizard               !!*//

//Code for Swept Away VEX Challenge CTD: By Archan Das, Adam Kalman,and Theo Gikas. Anyone can use this code if they want to.
void drivercontrol(int LeftMotor, int RightMotor, int ArmMotor, int WristMotor)
	{
		while (true)
		{
			motor[RightMotor] = vexRT[Ch2];
			motor[LeftMotor] = vexRT[Ch3];
			motor[ArmMotor] = 0;
			if (vexRT[Ch5] == 0)
			{
				motor[ArmMotor] = 127;
			}
			else
			{
				motor[ArmMotor] = -127;
			}
			motor[ArmMotor] = 0;
			if (vexRT[Ch6] == 0)
			{
				motor[ActMotor] = 127;
			}
			else
			{
				motor[ActMotor] = -127;
			}
		}
	}
int t = 0;


task main()
{

		clearTimer(T1);
		while (t < 15)
		{
			motor[LeftDrive] = 50;
			motor[RightDrive] = 50;
			wait1Msec(100);
			motor[ArmMotor] = 127;
			while(SensorValue[Sonar] < 12 && SensorValue[LineFollower]<1500)
			{
				motor[LeftDrive] = 127;
				motor[RightDrive] = 127;
			}
					t = time1[T1];
		}


		drivercontrol(LeftDrive, RightDrive, ArmMotor, ActMotor);

}
